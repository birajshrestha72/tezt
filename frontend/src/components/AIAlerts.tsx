    ...
    <div className="space-y-8 p-4 sm:p-6 lg:p-8 bg-bg-primary min-h-screen">
      <section className="flex flex-col gap-5 rounded-3xl border border-border-light bg-surface-container/70 p-5 shadow-xl shadow-black/10 backdrop-blur md:p-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-3">
          <div>
            ...
            <h1 className="mt-1 text-3xl font-black tracking-tight text-text-primary sm:text-4xl">Operational alert dashboard</h1>
      ...
      <section className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {(loading ? [1, 2, 3, 4] : metrics).map((metric, index) => (
          <article key={loading ? index : metric.label} className="rounded-3xl border border-border-light bg-surface-container/70 p-5 shadow-lg shadow-black/10 transition-all duration-base hover:-translate-y-0.5 hover:border-brand-default/30">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-text-tertiary">{loading ? 'Loading' : metric.label}</p>
                <h2 className="mt-3 text-3xl font-black tracking-tight text-text-primary">{loading ? '—' : metric.value}</h2>
                <p className="mt-2 text-sm text-text-tertiary">{loading ? 'Gathering prediction inputs' : 'Live dashboard metric'}</p>
              </div>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1.35fr_0.95fr]">
        <div className="rounded-3xl border border-border-light bg-surface-container/70 shadow-xl shadow-black/10">
          <div className="flex flex-col gap-3 border-b border-border-light px-5 py-4 md:flex-row md:items-center md:justify-between md:px-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-text-tertiary">Prediction alerts</p>
                <h3 className="mt-1 text-lg font-bold text-text-primary">Severity and recommendation feed</h3>
              </div>
            </div>
          ...
          <div className="divide-y divide-border-light/60">
            {loading ? (
              <div className="px-5 py-8 text-sm text-text-tertiary md:px-6">Loading AI alerts...</div>
            ) : alerts.length > 0 ? (
              alerts.map((alert) => (
                <div key={alert.id} className="flex flex-col gap-4 px-5 py-4 md:flex-row md:items-start md:justify-between md:px-6">
                  ...
                  <div className="max-w-xl rounded-2xl border border-border-light bg-white/5 p-4 text-sm leading-6 text-text-secondary">
              ...
              <div className="px-5 py-10 md:px-6">
                <div className="rounded-3xl border border-dashed border-border-light bg-bg-secondary px-5 py-10 text-center">
                  ...
                  <h3 className="mt-4 text-lg font-bold text-text-primary">No AI alerts available.</h3>
        ...
        <aside className="rounded-3xl border border-border-light bg-surface-container/70 p-5 shadow-xl shadow-black/10 md:p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-text-tertiary">Insight summary</p>
              <h3 className="mt-1 text-lg font-bold text-text-primary">Current AI note</h3>
            </div>
            <BrainCircuit className="h-5 w-5 text-primary" />
          </div>

          <div className="mt-5 space-y-3">
            <div className="rounded-2xl border border-border-light bg-white/5 p-4 text-sm leading-6 text-text-secondary">
